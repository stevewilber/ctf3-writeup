...
func (s *Server) sqlHandler(w http.ResponseWriter, req *http.Request) {
	...
	if state != "primary" {
		...
		// return
		body, err := s.client.SafePost(s.cluster.primary.ConnectionString, "/sql", bytes.NewReader(query))
		if err != nil {
			log.Printf("Couldn't proxy query to %v: %s", s.cluster.primary.ConnectionString, err)
			// time.Sleep(2000 * time.Millisecond)
			http.Error(w, err.Error(), http.StatusBadRequest)
			// time.Sleep(100000 * time.Millisecond)
		} else {
			resp, err2 := ioutil.ReadAll(body)
			if (err2 != nil) {
				log.Printf("Couldn't proxy query (err2) to %v: %s", s.cluster.primary.ConnectionString, err2)
				// time.Sleep(2000 * time.Millisecond)
	...
	//going to try proxying to master, so dont bother replicating
	// r := &Replicate{
	// 	Self:  s.cluster.self,
	// 	Query: query,
	// }
